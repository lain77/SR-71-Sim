...
void main() {
    ...
    // ── Sol no céu — apenas um ponto pequeno ─────────────
    sky += vec3(1.0, 0.98, 0.93) * pow(sunDot, 4000.0) * 2.0;

    // ── Aquecimento solar — APENAS a baixa altitude ──────
    float lowAltOnly = 1.0 - darkening;
    float sunInfluence = pow(sunDot, 3.0) * clamp(1.0 - upDot * 2.5, 0.0, 1.0);
    sky += vec3(1.0, 0.78, 0.40) * sunInfluence * 0.12 * lowAltOnly;

    // ── Banda de atmosfera no horizonte ──────────────────
    if (altFactor > 0.2) {
        float bandStrength = (altFactor - 0.2) / 0.8;
        float bandArea = pow(clamp(1.0 - abs(upDot + 0.02), 0.0, 1.0), 20.0);
        // APENAS azul — sem mistura com cores quentes
        sky += vec3(0.15, 0.30, 0.75) * bandArea * bandStrength * 0.6;
    }
